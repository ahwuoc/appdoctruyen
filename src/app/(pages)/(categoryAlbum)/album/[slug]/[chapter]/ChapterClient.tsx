...
export default function ChapterClient({ params }: { params: Promise<{ slug: string; chapter: string; }>; }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const { data: album, error: albumErr } = await supabase
          .from("albums")
          .select("id, title, image_url, chapters(id, title, order_sort, album_id)")
          .eq("id", albumId)
          .single();
    ...
    fetchData();
    window.scrollTo(0, 0);
  }, [chapterId, albumId, addToHistory, resolvedParams.slug]);
